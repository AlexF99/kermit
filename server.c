...
int main(int argc, char const *argv[])
{
    ...
    for (;;)
    {
        ...
        else if (msg_in && msg_in->tipo == MD5_T)
        {
            char nome_arquivo[100];
            strcpy(nome_arquivo, (char*)msg_in->dados);
            printf("%s\n", nome_arquivo);
            ...
            if (!arq)
            {
                ...
                strcat(error_msg, nome_arquivo);
                msg_out = cria_mensagem(sizeof(error_msg), msg_in->sequencia+1, ERRO, (unsigned char *) error_msg);
                envia_mensagem(msg_out, buffer_out, socket);
            }
            else
            {
                ...
                char result[63];
                if (strcmp((char *)msg_in->dados, (char*)hash) == 0)
                    strcpy(result, "MD5 correto.\n");
                else
                    strcpy(result, "MD5 incorreta.\n");

                msg_out = cria_mensagem(strlen(result), msg_in->sequencia+1, DADOS, (unsigned char *)result);
                envia_mensagem(msg_out, buffer_out, socket);
                printf("enviou\n");
                fclose(arq);
            }
